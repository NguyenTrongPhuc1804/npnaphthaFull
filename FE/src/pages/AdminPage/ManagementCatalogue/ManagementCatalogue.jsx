...
import FormCreateCatalogue from "../../../components/Form/Catalogue/FormCreateCatalogue";
import {
  deleteAllCatalogue,
  deleteCatalogue,
  getAllCatalogue,
...
import FormUpdateCatalogue from "../../../components/Form/Catalogue/FormUpdateCatalogue";
...
const TABLE_HEAD = ["", "Tên catalogue", "Đường dẫn tĩnh", "Ngày khởi tạo", ""];
...
export default function ManagementCatalogue() {
  ...
  const handleSearchCate = () => {
    dispatch(searchCatalogue({ searchBy, searchValue }));
    setSearchValue("");
  };
  const handleOpenForm = () => {
    dispatch(
      openModal({ body: <FormCreateCatalogue />, title: "Tạo catalogue mới" })
    );
  };
  ...
  const handleDeleteAllProduct = () => {
    dispatch(deleteAllCatalogue(listDelete));
    setListDelete([]);
  };
  useEffect(() => {
    ...
    <Card className="h-full w-full">
      <CardHeader floated={false} shadow={false} className="rounded-none">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-8">
          <div>
            ...
            <Typography color="gray" className="mt-1 font-normal">
              Xem thông tin về tất cả catalogue
            </Typography>
          </div>
        ...
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          ...
          <div className="w-full md:w-72">
            <Input
              placeholder="tìm kiếm theo tên catalogue"
              label="Search"
              value={searchValue}
      ...
      <CardBody className="overflow-scroll px-0">
        <table className="mt-4 w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {listAllCatalogue?.data?.map((item, index) => {
              const isLast = index === listAllCatalogue.length - 1;
              ...
              return (
                <tr key={item._id}>
                  ...
                  <td className={classes}>
                    <div className="flex items-center gap-3   truncate ... w-[250px] ">
                      <img
                  ...
                  <td className={classes}>
                    <Tooltip content="Sửa catalogue">
                      <IconButton
                        onClick={() =>
                          dispatch(
                            openModal({
                              title: "Cập nhật catalogue",
                              body: <FormUpdateCatalogue data={item} />,
                            })
                          )
                        }
                        variant="text"
                      ...
                      <Popover>
                        ...
                        <PopoverContent>
                          <div className="flex items-center ">
                            <p className="text-sm mr-2">Xóa catalogue này??</p>
      ...
      <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
        <div className="w-full flex justify-center">
          <DefaultPagination
            ...
            e={(value) => {
              dispatch(getAllCatalogue({ page: value, limit: 8 }));
